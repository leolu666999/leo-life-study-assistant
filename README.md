...
- 公开产品首页：未登录访客可在悉尼大学、悉尼海港大桥和悉尼歌剧院高清实景轮播中了解任务、日程、收支和文件功能；右上角提供 `中 / EN` 整页切换以及登录、注册入口，私人页面仍受 Auth 保护。
- 数据不进 Git：`.gitignore` 会排除数据库、备份和上传文件，更新代码不会覆盖个人数据。
- 每日 To Do List：按日期管理每日清单，支持勾选和编辑；计划页使用 Daily / Weekly / Monthly 单视图切换，Daily 每行预览四张清单，并可按待办关键词或 `7月19日`、`7.19`、`2026.7.19` 等日期格式搜索。
...
- 课表日历：日、周视图使用时间轴，月视图使用周一到周日的日历网格，并按悉尼时间显示每节课；点击单次课程卡片可查看时间、地点、课次序号、校区和备注。
- Task / Deadline：长期任务和明确截止事项分开管理，Deadline 支持倒计时显示。首页只显示未完成、未归档任务；已完成记录保留在任务中心，可在“已完成”或“全部”中查看和恢复。
...
- 语言偏好：登录页可通过“中 / EN”即时切换中英文；设置页提供“简体中文 / 繁體中文 / English”三段选择，并把偏好保存到本地或当前云端账号。English 模式会完整翻译导航、表单、空状态、连接提示、日期和动态统计；繁體中文模式会完整使用繁体中文；简体中文模式则有意保留 `Today’s Schedule`、`Task Card`、`To Do List`、`Task` 和 `Deadline` 等核心产品词段。用户自行输入的标题、备注和文件内容不会被自动翻译。
...
登录页每次打开默认显示中文，并可在右上角切换为英文。English 模式的系统界面完整显示英文，繁體中文模式完整显示繁体中文；简体中文模式保留 `MyAssist`、`Today’s Schedule`、`Task Card`、`Task`、`Deadline` 和 `To Do List` 等既有英文产品词段。英文与繁体模式的日期输入同时支持键盘输入 `YYYY-MM-DD` 和日历选择。
