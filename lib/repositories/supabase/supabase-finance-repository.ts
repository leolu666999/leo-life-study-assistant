...
export const supabaseFinanceRepository: FinanceRepository = {
  ...
  async createExpense(input, context) {
    ...
    const expense = { type: input.type === "income" ? "income" : "expense",
    ...
    const { error } = await client.rpc("save_expense_with_currency", { p_expense_id: id, p_create: true, p_expense: expense });
    if (error) throw error;
  ...
  async updateExpense(id, input, context) {
    ...
    const expense = { type: next.type === "income" ? "income" : "expense", title: next.title,
